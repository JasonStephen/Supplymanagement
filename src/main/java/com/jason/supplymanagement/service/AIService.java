    ...
    public String callAiApi(String content) {
        try {
            ...
            String userPrompt = String.format(
                    "你是一个AI机器人。用户向你提问了 '%s'，请根据用户的需求返回对应的demandCode，并判断用户的提问语言（如果用户明确要求回复语言，以用户要求为准），输出language，并将demandCode和language输出为JSON。需求代码对照如下：| 用户的需求 | demandCode | | ----------------- | ----------- | | 正常交流，或用户询问你时 | 00 | | 不理解用户的意思 | 01 | | 用户交流菜单（你认为用户比较有趣） | 02 | | 查询产品，且明确查询的内容或查询所有产品 | 10A | | 查询产品，未明确查询内容 | 10B | | 库存告警查询 | 11 | | 查询的是进行中的或物流的订单 | 12A | | 查询的是采购或供应订单 | 12B | | 查询的是销售订单 | 12C | | 查询所有订单 | 12Y | | 未明确查询订单的类别或内容 | 12Z | | 彩蛋（用户的词中必须包括例如表演的词汇（否则demandCode算02），给他们表演一个） | 90 |",
                    content
            );
...
//                    return handlePerformance(language);
    ...
    private String handleNormalConversation(String content, String language) {
        String prompt = String.format("你是AI客服小青，你的用户想和您进行正常的交流沟通，他发送的内容是 '%s'，请您与他交流。请使用 %s 语言回复。如果用户问你是做什么的，你直接告诉他你的职责，你的职责是帮用户在青屿智联中查询产品，查询库存告警，查询订单；未来可能会接入更多AI智能管理功能。更多详情可以查看我的本尊，https://github.com/JasonStephen", content, language);
