...
-- 7. 产品类别表 (Product_Category) - 自引用外键，需要先创建
...
CREATE TABLE IF NOT EXISTS Product (
    ...
    category_id INT,
    price DECIMAL(10, 2) NOT NULL, -- 修改为 DECIMAL 类型
    FOREIGN KEY (category_id) REFERENCES Product_Category(category_id)
    );

-- 9. 采购订单表 (Purchase_Order) - 依赖 Supplier 和 Product 表
...
-- 11. 销售订单表 (Sales_Order) - 依赖 Customer 和 Product 表
...
-- 16. 物流订单表 (Logistics_Order) - 依赖 Purchase_Order, Sales_Order, Logistics_Company 表
...
-- 17. 物流协议表 (Logistics_Agreement) - 依赖 Logistics_Company 表
